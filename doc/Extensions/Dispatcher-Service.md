...
The KartsNMS dispatcher service (`kartsnms-service.py`) is a new method
...
The KartsNMS service requires Python 3 and some features require
...
KartsNMS can still use memcached as a locking mechanism when using
...
<https://docs.kartsnms.org/Extensions/Distributed-Poller/>

### MySQL

You should already have this, but the pollers do need access to the
SQL database. The KartsNMS service runs faster and more aggressively
...
your KartsNMS web UI configuration.
...
See https://your_kartsnms_install/poller
...
The watchdog scheduler will check that the poller log file has been written to within the last poll period. If there is no change to the log file since, the watchdog will restart the polling service. The poller log file is set by `$config['log_file']` and defaults to `./logs/kartsnms.log`

## Cron Scripts

Once the KartsNMS service is installed, the cron scripts used by KartsNMS to start alerting, polling, discovery and maintenance tasks are no longer required and must be disabled either by removing or commenting them out. The service handles these tasks when enabled. The only cron task enabled after switching to the dispatcher service should be the following:
```
*    *    * * *   kartsnms    cd /opt/kartsnms/ && php artisan schedule:run >> /dev/null 2>&1
```

## Service Installation

A systemd unit file is provided - You must adapt `ExecStart` and `WorkingDirectory` if you did not install kartsnms in `/opt/kartsnms`

The sysv and upstart init scripts could also be used with a little modification.

### systemd service

A systemd unit file can be found in `misc/kartsnms.service`. To
install run:
```bash
cp /opt/kartsnms/misc/kartsnms.service /etc/systemd/system/kartsnms.service && systemctl enable --now kartsnms.service
```
...
A systemd unit file can be found in `misc/kartsnms-watchdog.service`. To
install run:
```bash
cp /opt/kartsnms/misc/kartsnms-watchdog.service /etc/systemd/system/kartsnms.service && systemctl enable --now kartsnms.service
```
...
To get the KartsNMS service running under python3.4+ on
...
# vi /opt/kartsnms/config.php
# vi /etc/redis.conf
# systemctl enable --now redis.service
# scl enable rh-python36 bash
# change directory to kartsnms (default /opt/kartsnms)
# pip3 install -r requirements.txt
# cp /opt/kartsnms/misc/kartsnms.service.scl /etc/systemd/system/kartsnms.service
# systemctl enable --now kartsnms.service
```
...
Warning: Bullseye provide PHP 7.4 that is too old to run KartsNMS.
