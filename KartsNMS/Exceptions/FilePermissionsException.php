...
use KartsNMS\Config;
use KartsNMS\Interfaces\Exceptions\UpgradeableException;
...
class FilePermissionsException extends \Exception implements UpgradeableException
{
    ...
    {
        $log_file = config('app.log') ?: Config::get('log_file', base_path('logs/kartsnms.log'));
    ...
    {
        $user = config('kartsnms.user');
        $group = config('kartsnms.group');
        ...
        // check if webserver is in the kartsnms group
