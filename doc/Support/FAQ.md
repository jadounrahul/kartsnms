...
### <a name="faq1"> How do I install KartsNMS?</a>

This is currently well documented within the doc folder of the installation files.

Please see the following [doc](/Installation/Install-KartsNMS.md)

### <a name="faq2"> How do I add a device?</a>

You have two options for adding a new device into KartsNMS.

1: Using the command line via ssh you can add a new device by changing
   to the directory of your KartsNMS install and typing:
...
### <a name="faq4"> What are the supported OSes for installing KartsNMS on?</a>
...
We do indeed, you can find access to the demo [here](https://demo.kartsnms.org)


## Support

### <a name='how-does-kartsnms-use-mibs'>How does KartsNMS use MIBs?</a>

KartsNMS does not parse MIBs to discover sensors for devices.
KartsNMS uses static discovery definitions written in YAML or PHP.
Therefore, updating a MIB alone will not improve OS support, the
definitions must be updated.  KartsNMS only uses MIBs to make OIDs
easier to read.
...
kartsnms from within your install directory. This should give you info on
...
using the [included
snmpd.conf](https://raw.githubusercontent.com/kartsnms/kartsnms/master/snmpd.conf.example)
file.
...
If this is related to your web service for KartsNMS then this has been
...
443](https://github.com/jadounrahul/kartsnms/issues/443).
...
see [RRDTune doc](../Extensions/RRDTune.md)
 ...
 at 0 (at device boot) and count up from there. KartsNMS records the
...
There is a host rename tool called renamehost.php in your kartsnms
...
### <a name="faq24"> How do I migrate my KartsNMS install to another server?</a>
...
scripts](https://web.archive.org/web/20180815212723/https://vlan50.com/2015/04/17/migrating-from-observium-to-kartsnms/).
...
- Install KartsNMS as per our normal documentation; you don't need to
  run through the web installer or building the sql schema.
- Stop cron by commenting out all lines in `/etc/cron.d/kartsnms`
- Dump the MySQL database `kartsnms` from your old server (`mysqldump
  kartsnms -u root -p > kartsnms.sql`)...
- and import it into your new server (`mysql -u root -p kartsnms < kartsnms.sql`).
...
- Ensure ownership of the copied files and folders (substitute your
  user if necessary) - `chown -R kartsnms:kartsnms /opt/kartsnms`
- Delete old pollers on the GUI (gear icon --> Pollers --> Pollers)
- Validate your installation (/opt/kartsnms/validate.php)
- Re-enable cron by uncommenting all lines in `/etc/cron.d/kartsnms`
...
issue](https://github.com/jadounrahul/kartsnms/issues/new).
...
Restart snmpd and KartsNMS should populate the additional disk after a fresh discovery.
...
this from the KartsNMS directory:
...
update KartsNMS with those new details.
Please edit `.env`
...
make sure the timezone is set properly on both the KartsNMS **and**
...
to be delivered late if KartsNMS is on a timezone later than
MySQL/MariaDB.
...
As of [Ver. 1.55](https://community.itkarts.com/t/v1-55-release-changelog-august-2019/9428) a new feature has been added where you can view a simple linear prediction in port graphs.
...
There is already a reference how to move your whole LNMS installation to another server. But the following steps will help you to split up an "All-in-one" installation to one KartsNMS installation with a separate database install.
*Note: This section assumes you have a MySQL/MariaDB instance

- Stop the apache and mysql service in you KartsNMS installation.
- Edit out all the cron entries in `/etc/cron.d/kartsnms`.
- Dump your `kartsnms`database on your current install by issuing `mysqldump kartsnms -u root -p > kartsnms.sql`.
...
- Copy this over to your new database server and import it with `mysql -u root -p kartsnms < kartsnms.sql`.
- Enter to mysql and add permissions with the following two commands:
```sql
GRANT ALL PRIVILEGES ON kartsnms.* TO 'kartsnms'@'IP_OF_YOUR_LNMS_SERVER' IDENTIFIED BY 'PASSWORD' WITH GRANT OPTION;
GRANT ALL PRIVILEGES ON kartsnms.* TO 'kartsnms'@'FQDN_OF_YOUR_LNMS_SERVER' IDENTIFIED BY 'PASSWORD' WITH GRANT OPTION;
FLUSH PRIVILEGES;
exit;
```
- Enable and restart MySQL server.
- Edit your `config.php` file to point the install to the new database server location.
- **Very important**: On your KartsNMS server, inside your install directory is a `.env` file, in it you need to edit the `DBHOST` paramater to point to your new server location.
...
Please [open a feature request in the community forum](https://community.itkarts.com/c/feature-requests) and provide
the output of Discovery, Poller, and Snmpwalk as separate non-expiring
<https://p.kartsn.ms/> links :
...
> These commands will automatically upload the data to KartsNMS servers.
...
- Code. This is a big thing. We want this community to grow by the
  ...
  - Poller / Discovery code. Improving it (we think a lot can be done
    ...
    ones.
  - The KartsNMS main website, this is hosted on GitHub like the main
...
- Feature requests. Can't code / won't code. No worries, chuck a
  ...
  forum](https://community.itkarts.com) with enough detail and
...
KartsNMS can and is developed by anyone, this means someone may be
...
```bash
git remote add f0o https://github.com/f0o/kartsnms.git
