...
foreach ($heads as $head => $extra) {
    ...
    echo '<th><a href="' . \KartsNMS\Util\Url::generate(['page' => 'device', 'device' => $device['device_id'], 'tab' => 'processes', 'order' => $lhead, 'by' => $bhead]) . '"><span' . $icon . '>&nbsp;';
    if (! empty($extra)) {
...
foreach (dbFetchRows('SELECT * FROM `processes` WHERE `device_id` = ? ORDER BY ' . $order . ' ' . $by, [$device['device_id']]) as $entry) {
    ...
    echo '<td>' . \KartsNMS\Util\Number::formatSi($entry['vsz'] * 1024, 2, 3, '') . '</td>';
    echo '<td>' . \KartsNMS\Util\Number::formatSi($entry['rss'] * 1024, 2, 3, '') . '</td>';
