...
if (count($drives)) {
    ...
    foreach ($drives as $drive) {
        ...
        if ($device['os'] == 'junos') {
            foreach (\KartsNMS\Config::get('ignore_junos_os_drives', []) as $jdrive) {
        ...
        if ($device['os'] == 'freebsd') {
            foreach (\KartsNMS\Config::get('ignore_bsd_os_drives', []) as $jdrive) {
                if (preg_match($jdrive, $drive['storage_descr'])) {
                    $skipdrive = 1;
                }
            }
        ...
        $background = \KartsNMS\Util\Color::percentage($percent, $drive['storage_perc_warn']);
        ...
        $drive['storage_descr'] = \KartsNMS\Util\StringHelpers::shortenText($drive['storage_descr'], 50);
        ...
        echo '<tr>
           <td class="col-md-4">' . \KartsNMS\Util\Url::overlibLink($link, $drive['storage_descr'], $overlib_content) . '</td>
