...
class Dispatcher
{
    ...
    public static function handle(Trap $trap): bool
    {
        ...
        if ($trap->findOid('iso.3.6.1.6.3.1.1.4.1.0')) {
            ...
            // KartsNMS snmptraps code cannot process received data. Let's inform the user.
            Eventlog::log('Misconfigured MIBS or MIBDIRS for snmptrapd, check https://docs.kartsnms.org/Extensions/SNMP-Trap-Handler/ : ' . $trap->raw, $trap->getDevice(), 'system');

            return false;
        }

        // note, this doesn't clear the resolved SnpmtrapHandler so only one per run
        /** @var \KartsNMS\Interfaces\SnmptrapHandler $handler */
        $handler = app(\KartsNMS\Interfaces\SnmptrapHandler::class, [$trap->getTrapOid()]);
