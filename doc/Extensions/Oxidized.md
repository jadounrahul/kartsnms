...
Then you can procede to the KartsNMS Web UI and go to Oxidized
...
oxidized to pull them from KartsNMS [Feeding
Oxidized](#feeding-oxidized) Note: this means devices will be controlled by
the KartsNMS API, and not router.db, passwords will still need to be in the
...
KartsNMS supports config versioning if Oxidized does.  This is known
...
Group level username/password or per device. KartsNMS currently
...
support for Oxidized has been added to the KartsNMS API. A sample
config for Oxidized is provided below.

You will need to configure default credentials for your devices in the
Oxidized config, KartsNMS doesn't provide login credentials at this
time.
...
```bash
      source:
        ...
        debug: false
        http:
          url: https://kartsnms/api/v0/oxidized
          map:
            name: hostname
            model: os
            group: group
...
KartsNMS is able to reload the Oxidized list of nodes, each time a
device is added to KartsNMS. To do so, edit the option in Global
...
finally by defining the overriding value itself. KartsNMS does not
...
When you're disabling or removing a device from KartsNMS, the
configuration will no longer be available via the KartsNMS web interface.  
...
If you want to purge saved config of a device that is not in KartsNMS anymore, you can run the following command:
