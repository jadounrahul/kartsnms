...
"""
 ...
 Authors:       Orsiris de Jong <contact@netpower.fr>
                Neil Lathwood <neil@kartsnms.org>
...
import KartsNMS
from KartsNMS.command_runner import command_runner
...
def wrapper(
    ...
    **kwargs,  # Type: dict, may contain modules
    ...
    db_connection = KartsNMS.DB(sconfig)
...
if __name__ == "__main__":
    parser = ArgumentParser(
        ...
        description="Spawn multiple kartsnms php processes in parallel.",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    config = KartsNMS.get_config_data(
        os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    )
    log_dir = config["log_dir"]
    log_file = os.path.join(log_dir, wrapper_type + ".log")
    logger = KartsNMS.logger_get_logger(log_file, debug=debug)
