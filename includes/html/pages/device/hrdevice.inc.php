...
foreach (dbFetchRows('SELECT * FROM `hrDevice` WHERE `device_id` = ? ORDER BY `hrDeviceIndex`', [$device['device_id']]) as $hrdevice) {
    ...
    if ($hrdevice['hrDeviceType'] == 'hrDeviceProcessor') {
        ...
        $proc_popup .= "</div><img src=\'graph.php?id=" . $proc_id . '&amp;type=processor_usage&amp;from=' . \KartsNMS\Config::get('time.month') . '&amp;to=' . \KartsNMS\Config::get('time.now') . "&amp;width=400&amp;height=125\'>";
        $proc_popup .= "', RIGHT" . \KartsNMS\Config::get('overlib_defaults') . ');" onmouseout="return nd();"';
        ...
        $mini_graph = \KartsNMS\Util\Url::overlibLink($proc_url, \KartsNMS\Util\Url::lazyGraphTag($graph_array), \KartsNMS\Util\Url::graphTag($graph_array_zoom));
        ...
        if ($interface['ifIndex']) {
            ...
            $mini_graph = \KartsNMS\Util\Url::overlibLink(generate_port_url($interface), \KartsNMS\Util\Url::lazyGraphTag($graph_array), \KartsNMS\Util\Url::graphTag($graph_array_zoom));
