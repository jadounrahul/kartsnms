...
class IRCBot
{
    ...
    {
        $container_dir = '/data';
        if (file_exists($container_dir) and posix_getpwuid(fileowner($container_dir))['name'] == 'kartsnms') {
    ...
    private function _auth($params)
    {
        ...
        if (strlen($params[0]) == 64) {
            ...
            if ($user->email && $user->username == $params[0]) {
                ...
                if (Mail::send($user->email, 'KartsNMS IRC-Bot Authtoken', "Your Authtoken for the IRC-Bot:\r\n\r\n" . $token . "\r\n\r\n", false) === true) {
