    ...
    if (is_array($oids)) {
        foreach ($oids as $index => $entry) {
            ...
            if ($entitysensor[$entry['entSensorType']] && is_numeric($entry['entSensorValue']) && is_numeric($index)) {
                ...
                // If temperature sensor, set low thresholds to -1 and -5. Many sensors don't return low thresholds, therefore KartsNMS takes the runtime low
                // Also changing 0 values (not just null) as Karts loses these somewhere along the line and shows an empty value in the Web UI
                if ($type == 'temperature') {
                    if ($warn_limit_low == 0) {
                        $warn_limit_low = -1;
                    }
