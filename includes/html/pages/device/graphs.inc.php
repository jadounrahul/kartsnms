...
// Graphs are printed in the order they exist in \KartsNMS\Config::get('graph_types')
$link_array = [
    'page'   => 'device',
...
foreach (dbFetchRows('SELECT * FROM device_graphs WHERE device_id = ? ORDER BY graph', [$device['device_id']]) as $graph) {
    $section = \KartsNMS\Config::get("graph_types.device.{$graph['graph']}.section");
    if ($section != '') {
        ...
        if ($graph_enable[$graph]) {
            if ($graph == 'customoid') {
                foreach (dbFetchRows('SELECT * FROM `customoids` WHERE `device_id` = ? ORDER BY `customoid_descr`', [$device['device_id']]) as $graph_entry) {
                    $graph_title = \KartsNMS\Config::get("graph_types.device.$graph.descr") . ': ' . $graph_entry['customoid_descr'];
                ...
                $graph_title = \KartsNMS\Config::get("graph_types.device.$graph.descr");
