...
KartsNMS can be used with a MariaDB Galera Cluster. This is a Multi Master cluster, meaning each
node in the cluster can read and write to the database. They all have the same ability. KartsNMS will
...
wsrep_cluster_name="kartsnms_cluster"
...
KartsNMS supports up to 9 galera nodes, you define these nodes in the .env file. For each node we have the ability to define if this kartsnms installation/poller is able to write, read or both to that node.
The galera nodes you define here can be the same or differnt for each kartsnms poller. If you have a poller you only want to write/read to one galera node, you would simply add one DB_HOST, and omit all the rest. This allows you to precisely control what galera nodes a kartsnms poller is reading and or writing too.
...
```
The above .env on a kartsnms installation/poller would communicate to each galera node as follows.
