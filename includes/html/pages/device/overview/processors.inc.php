...
';
    ...
    foreach ($processors as $proc) {
        ...
        if (\KartsNMS\Config::get('cpu_details_overview') === true) {
            $background = \KartsNMS\Util\Color::percentage($percent, $proc['processor_perc_warn']);
            ...
            echo '<tr>
                <td class="col-md-4">' . \KartsNMS\Util\Url::overlibLink($link, $text_descr, $overlib_content) . '</td>
    ...
    if (\KartsNMS\Config::get('cpu_details_overview') === false) {
        ...
        foreach ($total_percent as $type => $values) {
            ...
            $background = \KartsNMS\Util\Color::percentage($percent_usage, $percent_warn);

            echo '
              <tr>
                <td class="col-md-4">' . \KartsNMS\Util\Url::overlibLink($link, $values['descr'], $overlib_content) . '</td>
                <td class="col-md-4">' . \KartsNMS\Util\Url::overlibLink($link, 'x' . $values['count'], $overlib_content) . '</td>
                <td class="col-md-4">' . \KartsNMS\Util\Url::overlibLink($link, print_percentage_bar(200, 20, $percent_usage, null, 'ffffff', $background['left'], $percent_usage . '%', 'ffffff', $background['right']), $overlib_content) . '</td>
              </tr>';
        }
    }
