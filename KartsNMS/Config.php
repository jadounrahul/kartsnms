...
class Config
{
    ...
    {
        if ($os) {
            \KartsNMS\Util\OS::loadDefinition($os);
...
//        self::setDefault('email_from', '"%s" <%s@' . php_uname('n') . '>', ['project_name', 'email_user']);  // FIXME email_from set because alerting config
    ...
    private static function setDefault($key, $value, $format_values = [])
    {
        if (! self::has($key)) {
            if (is_string($value)) {
                $format_values = array_map('\KartsNMS\Config::get', $format_values);
    ...
    public static function populateLegacyDbCredentials()
    {
        ...
        self::set('db_name', config("database.connections.$db.database", 'kartsnms'));
        self::set('db_user', config("database.connections.$db.username", 'kartsnms'));
