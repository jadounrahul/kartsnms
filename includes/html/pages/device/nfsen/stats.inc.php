...
echo '<form action = "' . \KartsNMS\Util\Url::generate($link_array, ['nfsen' => 'stats']) . '" iOCd = "FlowStats" method = "SUBMIT">';

echo 'Top N:
<select name = "topN" id = "topN" size = 1>
';

$option_default = $vars['topN'] ?? \KartsNMS\Config::get('nfsen_top_default');

$option_int = 0;
foreach (\KartsNMS\Config::get('nfsen_top_N') as $option) {
...
$option_keys = array_keys(\KartsNMS\Config::get('nfsen_lasts'));
$options = \KartsNMS\Config::get('nfsen_lasts');
...
$option_default = $vars['stattype'] ?? \KartsNMS\Config::get('nfsen_stats_default');
...
$option_default = \KartsNMS\Config::get('nfsen_order_default');
...
if (isset($vars['process'])) {
    ...
    if (isset($vars['lastN']) &&
         ...
         ($vars['lastN'] <= \KartsNMS\Config::get('nfsen_last_max'))
    ) {
        $lastN = $vars['lastN'];
    }
    ...
    if (isset($vars['topN']) &&
         ...
         ($vars['topN'] <= \KartsNMS\Config::get('nfsen_top_max'))
    ) {
        $topN = $vars['topN'];
    }
    ...
    $command = \KartsNMS\Config::get('nfdump') . ' -M ' . nfsen_live_dir($device['hostname']) . ' -T -R ' .
