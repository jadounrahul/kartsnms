 ...
 * @copyright 2020 Adam Bishop, KartsNMS
 * @license GPL
 */

namespace KartsNMS\Alert\Transport;
...
class Sensu extends Transport
{
    ...
    private function generateData(array $alert_data, int $status, int $offset = 0): array
    {
        ...
        return [
            'check' => [
                ...
                'command' => sprintf('KartsNMS: %s', $alert_data['builder']),
    ...
    private function generateAnnotations(array $alert_data): array
    {
        return array_filter([
            ...
            'kartsnms-notes' => $alert_data['notes'],
            'kartsnms-device-id' => strval($alert_data['device_id']),
            'kartsnms-rule-id' => strval($alert_data['rule_id']),
            'kartsnms-status-reason' => $alert_data['status_reason'],
    ...
    {
        // Sensu only has a single short (status) to indicate both severity and status, so we need to map KartsNMS' state and severity onto it

        if ($state === AlertState::RECOVERED) {
            // KartsNMS alert is resolved, send ok
            return self::OK;
        }
    ...
    {
        // Shrink the last domain components - e.g. kartsnms.corp.example.net becomes kartsnms.cen
