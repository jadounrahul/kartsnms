...
foreach (dbFetchRows($sql, $param) as $processor) {
    ...
    $background = \KartsNMS\Util\Color::percentage($perc, $processor['processor_perc_warn']);
    $bar_link = \KartsNMS\Util\Url::overlibLink($link, print_percentage_bar(400, 20, $perc, $perc . '%', 'ffffff', $background['left'], (100 - $perc) . '%', 'ffffff', $background['right']), \KartsNMS\Util\Url::graphTag($graph_array_zoom));
