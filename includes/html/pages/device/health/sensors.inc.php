...
foreach (dbFetchRows('SELECT * FROM `sensors` WHERE `sensor_class` = ? AND `device_id` = ? ORDER BY `sensor_descr`', [$class, $device['device_id']]) as $sensor) {
    ...
    $sensor_limit = trim(\KartsNMS\Util\Number::formatSi($sensor['sensor_limit'], 2, 3, '') . $unit);
    $sensor_limit_low = trim(\KartsNMS\Util\Number::formatSi($sensor['sensor_limit_low'], 2, 3, '') . $unit);
    $sensor_limit_warn = trim(\KartsNMS\Util\Number::formatSi($sensor['sensor_limit_warn'], 2, 3, '') . $unit);
    $sensor_limit_low_warn = trim(\KartsNMS\Util\Number::formatSi($sensor['sensor_limit_low_warn'], 2, 3, '') . $unit);
