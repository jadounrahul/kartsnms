 ...
 * OS.php
 *
 * OS related functions (may belong in KartsNMS/OS, but here for now)
 *
...
class OS
{
    ...
    {
        $install_dir = \KartsNMS\Config::get('install_dir');
        $cache_file = $install_dir . '/cache/os_defs.cache';
        if ($cached && is_file($cache_file) && (time() - filemtime($cache_file) < \KartsNMS\Config::get('os_def_cache_time'))) {
            // Cached
            ...
            \KartsNMS\Config::set('os', array_replace_recursive($os_defs, \KartsNMS\Config::get('os')));
        } else {
            // load from yaml
