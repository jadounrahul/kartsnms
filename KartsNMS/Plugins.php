...
class Plugins
{
    ...
    public static function scanNew()
    {
        ...
        if (file_exists(\KartsNMS\Config::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(\KartsNMS\Config::get('plugin_dir')), ['..', '.']);
            $plugin_files = array_diff($plugin_files, Plugin::versionOne()->pluck('plugin_name')->toArray());
            foreach ($plugin_files as $name) {
                if (is_dir(\KartsNMS\Config::get('plugin_dir') . '/' . $name)
                    && is_file(\KartsNMS\Config::get('plugin_dir') . '/' . $name . '/' . $name . '.php')) {
    ...
    public static function scanRemoved()
    {
        ...
        if (file_exists(\KartsNMS\Config::get('plugin_dir'))) {
            $plugin_files = array_diff(scandir(\KartsNMS\Config::get('plugin_dir')), ['.', '..', '.gitignore']);
