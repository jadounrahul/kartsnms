...
import KartsNMS
from KartsNMS.config import DBConfig
...
class ServiceConfig(DBConfig):
    def __init__(self):
        """
        Stores all of the configuration variables for the KartsNMS service in a common object
    ...
    watchdog_logfile = "logs/kartsnms.log"

    def populate(self):
        config = KartsNMS.get_config_data(self.BASE_DIR)
        ...
        self.watchdog_logfile = config.get("log_file", self.logdir + "/kartsnms.log")
...
class Service:
    ...
    def __init__(self):
        ...
        self._db = KartsNMS.DB(self.config)
        ...
        self.daily_timer = KartsNMS.RecurringTimer(
            self.config.update_frequency, self.run_maintenance, "maintenance"
        )
        self.stats_timer = KartsNMS.RecurringTimer(
        ...
        if self.config.watchdog_enabled:
            ...
            self.watchdog_timer = KartsNMS.RecurringTimer(
                self.config.poller.frequency, self.logfile_watchdog, "watchdog"
            )
        else:
            logger.info("Watchdog is disabled.")
        self.systemd_watchdog_timer = KartsNMS.RecurringTimer(
    ...
    def start(self):
        ...
        self.poller_manager = KartsNMS.PollerQueueManager(self.config, self._lm)
        self.queue_managers["poller"] = self.poller_manager
        self.discovery_manager = KartsNMS.DiscoveryQueueManager(self.config, self._lm)
        self.queue_managers["discovery"] = self.discovery_manager
        self.queue_managers["alerting"] = KartsNMS.AlertQueueManager(
            self.config, self._lm
        )
        self.queue_managers["services"] = KartsNMS.ServicesQueueManager(
            self.config, self._lm
        )
        self.queue_managers["billing"] = KartsNMS.BillingQueueManager(
            self.config, self._lm
        )
        self.queue_managers["ping"] = KartsNMS.PingQueueManager(self.config, self._lm)
        ...
        logger.info("KartsNMS Service: {} started!".format(self.config.unique_name))
        logger.info(
            "Poller group {}. Using Python {} and {} locks and queues".format(
                "0 (default)" if self.config.group == [0] else self.config.group,
                python_version(),
                "redis" if isinstance(self._lm, KartsNMS.RedisLock) else "internal",
            )
        )
        logger.info(
    ...
    def run_maintenance(self):
        ...
        exit_code, output = KartsNMS.call_script("daily.sh")
        if exit_code == 0:
