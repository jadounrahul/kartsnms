...
OS discovery is how KartsNMS detects which OS should be used for a device.
...
`bad_ifXEntry`: This is a list of models for which to tell KartsNMS
...
the discovery yaml `includes/definitions/discovery/<os>.yaml` or `KartsNMS/OS/<os>.php` if
...
At this step we should see all the values retrieved in KartsNMS.
