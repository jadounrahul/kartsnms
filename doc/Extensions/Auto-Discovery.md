...
KartsNMS provides the ability to automatically add devices on your
...
However, KartsNMS will only try to add the new devices discovered with LLDP/xDP if `$config['autodiscovery']['xdp'] = true;`.
...
Apart from the aforementioned Auto-Discovery options, KartsNMS is also
...
within your KartsNMS installation directory.
...
Scan network for snmp hosts and add them to KartsNMS.
