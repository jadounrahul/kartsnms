...
foreach (dbFetchRows('SELECT * FROM `customoids` WHERE `device_id` = ? ORDER BY `customoid_descr`', [$device['device_id']]) as $customoid) {
    ...
    $customoid_current = \KartsNMS\Util\Number::formatSi($customoid['customoid_current'], 2, 3, '') . $customoid_unit;
    $customoid_limit = \KartsNMS\Util\Number::formatSi($customoid['customoid_limit'], 2, 3, '') . $customoid_unit;
    $customoid_limit_low = \KartsNMS\Util\Number::formatSi($customoid['$customoid_limit_low'], 2, 3, '') . $customoid_unit;
