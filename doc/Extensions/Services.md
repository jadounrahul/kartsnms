...
Services within KartsNMS provides the ability to leverage Nagios plugins to
...
Service Templates within KartsNMS provides the same ability as Nagios
does with Host Groups. Known as Device Groups in KartsNMS.
...
This will point KartsNMS at the location of the nagios plugins -
...
file (/etc/cron.d/kartsnms typically) like:

```bash
*/5 * * * * kartsnms /opt/kartsnms/services-wrapper.py 1
```
...
poll the local KartsNMS server it's running on, regardless of which
...
separate graph. KartsNMS expects scripts to return using Nagios
...
```
    ...
    RRD[update /opt/kartsnms/rrd/localhost/services-26.rrd N:0.016:0:0.044:0.009]
    -- snip --
```
...
Change user to kartsnms for example
...
...to `/etc/check_mk/mrpe.cfg` on your remote host, you should be able to check its output by configuring a service using the [check_mrpe](https://raw.githubusercontent.com/kartsnms/kartsnms-agent/master/agent-local/check_mrpe) script.

 - Add [check_mrpe](https://raw.githubusercontent.com/kartsnms/kartsnms-agent/master/agent-local/check_mrpe) to the Nagios plugins directory on your KartsNMS server and make it executable.
- In KartsNMS, add a new service to the desired device with the type mrpe.
