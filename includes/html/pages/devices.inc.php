...
foreach ($menu_options as $option => $text) {
    ...
    $listoptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['format' => 'list_' . $option]) . '">' . $text . '</a>';
    if ($vars['format'] == 'list_' . $option) {
        $listoptions .= '</span>';
    }
    $sep = ' | ';
...
foreach ($menu_options as $option => $text) {
    ...
    $listoptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['format' => 'graph_' . $option, 'from' => '-24hour', 'to' => 'now']) . '">' . $text . '</a>';
    if ($vars['format'] == 'graph_' . $option) {
        $listoptions .= '</span>';
    }
    $sep = ' | ';
...
foreach (get_graph_subtypes($type) as $avail_type) {
    ...
    $headeroptions .= '<option value="' .
        \KartsNMS\Util\Url::generate($vars, [
            'format' => 'graph_' . $avail_type,
            'from' => $vars['from'] ?? \KartsNMS\Config::get('time.day'),
            'to' => $vars['to'] ?? \KartsNMS\Config::get('time.now'),
        ]) . '" ' . $is_selected . '>' . $display_type . '</option>';
}
$headeroptions .= '</select>';

if (isset($vars['searchbar']) && $vars['searchbar'] == 'hide') {
    $headeroptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['searchbar' => '']) . '">Restore Search</a>';
} else {
    $headeroptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['searchbar' => 'hide']) . '">Remove Search</a>';
}

$headeroptions .= ' | ';

if (isset($vars['bare']) && $vars['bare'] == 'yes') {
    $headeroptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['bare' => '']) . '">Restore Header</a>';
} else {
    $headeroptions .= '<a href="' . \KartsNMS\Util\Url::generate($vars, ['bare' => 'yes']) . '">Remove Header</a>';
}
...
if ($format == 'graph') {
    ...
    foreach (dbFetchRows($query, $sql_param) as $device) {
        ...
        if (device_permitted($device['device_id'])) {
            if (! $location_filter || $device['location'] == $location_filter) {
                ...
                $overlib_link = \KartsNMS\Util\Url::generate($link_array);

                echo '<div class="devices-overlib-box" style="min-width:' . ($width + 90) . '; max-width: ' . ($width + 90) . '">';
                echo '<div class="panel panel-default">';
                echo \KartsNMS\Util\Url::overlibLink($overlib_link, \KartsNMS\Util\Url::lazyGraphTag($graph_array_new), \KartsNMS\Util\Url::graphTag($graph_array_zoom));
                echo "</div></div>\n\n";
            }
        }
    }
    ...
    $os_selected = '""';
    if (isset($vars['os'])) {
        $os_selected = json_encode(['id' => $vars['os'], 'text' => \KartsNMS\Config::getOsSetting($vars['os'], 'text', $vars['os'])]);
    }
    ...
    <script>
        ...
        $(".devices-headers-table-menu").append(
            ...
            "<a href='<?php echo \KartsNMS\Util\Url::generate(array_diff_key($vars, ['_token' => 1])) ?>' title='Update the browser URL to reflect the search criteria.' class='btn btn-default'>Update URL</a>" +
            "<a href='<?php echo \KartsNMS\Util\Url::generate(['page' => 'devices', 'section' => $vars['section'] ?? '', 'bare' => $vars['bare'] ?? '']) ?>' title='Reset criteria to default.' class='btn btn-default'>Reset</a>" +
            "</form>" +
            "</div>"
        );
            <?php
