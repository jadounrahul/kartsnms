...
switch ($type) {
    case 'alerts':
        ...
        if (\KartsNMS\Config::get('alert.transports.mail') === true) {
        ...
        $transports = '';
        $x = 0;
        foreach (\KartsNMS\Config::get('alert.transports') as $name => $v) {
            if (\KartsNMS\Config::get("alert.transports.$name") === true) {
