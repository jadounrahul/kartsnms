...
use KartsNMS\Interfaces\ValidationGroup;
...
class Validator
{
    ...
    public function __construct()
    {
        ...
        $pattern = $this->getBaseDir() . '/KartsNMS/Validations/*.php';

        foreach (glob($pattern) as $file) {
            $class_name = basename($file, '.php');
            $class = '\KartsNMS\Validations\\' . $class_name;
    ...
    public function result(ValidationResult $result, string $group = null): void
    {
        ...
        if (empty($group)) {
            ...
            $bt = debug_backtrace();
            foreach ($bt as $entry) {
                if (Str::startsWith($entry['class'], 'KartsNMS\Validations')) {
                    $group = str_replace('KartsNMS\Validations\\', '', $entry['class']);
                    break;
                }
            }
        }
    ...
    /**
     * Execute a command, but don't run it as root.  If we are root, run as the KartsNMS user.
    ...
    public function execAsUser(string $command, array &$output = null, int &$code = null): void
    {
        if (self::getUsername() === 'root') {
            $command = 'su ' . \config('kartsnms.user') . ' -s /bin/sh -c "' . $command . '"';
        }
    ...
    /**
     * Get the base url for this KartsNMS install, this will only work from web pages.
